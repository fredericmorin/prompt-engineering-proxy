...
<template>
  <div class="rounded bg-muted p-3 text-xs font-mono leading-relaxed max-h-96 overflow-auto">
    <span v-if="tokens">{{ tokens }}</span>
    <span v-else class="text-muted-foreground">Waiting for tokens…</span>
    <span v-if="!done && !error" class="inline-block w-1.5 h-3 bg-current animate-pulse ml-0.5 align-middle" />
